...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        //List<UserMealWithExceed> list = getFilteredWithExceededcycle(mealList, LocalTime.of(7, 0), LocalTime.of(12,0), 2000);
    ...
    public static List<UserMealWithExceed>  getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> maps = mealList.stream().collect(Collectors.groupingBy(o -> o.getDateTime().toLocalDate(), Collectors.summingInt(UserMeal::getCalories)));

        return mealList.stream()
                .filter((s) -> TimeUtil.isBetween(s.getDateTime().toLocalTime(), startTime, endTime))
                .map((s) -> new UserMealWithExceed(s.getDateTime(),s.getDescription(),s.getCalories(),maps.get(s.getDateTime().toLocalDate())> caloriesPerDay))
